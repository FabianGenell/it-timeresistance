...
var Carousel = function (node) {
    ...
    const handleOverflow = (slides) => {
        ...
        setTimeout(() => {
            ...
            const needsOverflow =
                t$2(selectors$M.visibleSlides, swiperContainer).length !== slides.length;
...
class Product {
    ...
    onOptionChange(_ref2) {
        ...
        if (this.displayedDiscount) {
            const newDiscountEl =
                variant && n$2(selectors$G.displayedDiscountByVariantId(variant.id), this.container);
            ...
            .catch((error) => {
                ...
                if (error && error.message) {
                    if (typeof error.message === 'object') {
                        const sectionID = n$2(selectors$G.giftCardRecipientContainer, this.container)
                            .dataset.sectionId;
...
const filterDrawer = (node) => {
    ...
    const events = [
        ...
        e$2(t$2(''.concat(sel$2.button, ', ').concat(sel$2.clearAll), container), 'click', clickButton),
...
const filterBar = (node) => {
    ...
    function checkForActiveFilters() {
        const activeItems =
            containsCheckedInputs(t$2(sel$1.filterInputs, container)) || rangeInputsHaveValue();
...
const filterSidebar = (node) => {
    ...
    const events = [
        ...
        e$2(t$2(''.concat(sel.button, ', ').concat(sel.clearAll), container), 'click', clickButton),
        e$2(rangeInputs, 'change', rangeChanged)
    ];
    ...
    function checkForActiveFilters() {
        const activeItems =
            containsCheckedInputs(t$2(sel.filterInputs, container)) || rangeInputsHaveValue();
