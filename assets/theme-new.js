...
function forEachElementOrArray(n, t) {
    return Array.isArray(n) ? n.forEach(t) : t(n);
}
function createEventListenerAction(n) {
...
function createClassListAction(n) {
    return function (t) {
        var r = arguments;
        return forEachElementOrArray(t, function (t) {
            var c;
    ...
    Delegate.prototype.handle = function (event) {
        ...
        while (target && l) {
            for (i = 0; i < l; i++) {
                ...
                if (
                    target.tagName &&
                    ['button', 'input', 'select', 'textarea'].indexOf(target.tagName.toLowerCase()) >
                        -1 &&
...
const inventoryCounter = (container, config) => {
    ...
    const update = (variant) => {
        ...
        toggleClass(counterContainer, classes$p.hidden, !productIventoryValid(variantsInventories[variant.id]));
...
var Carousel = function (node) {
    ...
    const swiperContainer = hasClass(node, classes$o.carousel)
        ? node
        : qs(selectors$M.sliderContainer, node);
    ...
    const handleOverflow = (slides) => {
        ...
        setTimeout(() => {
            ...
            const needsOverflow =
                qsa(selectors$M.visibleSlides, swiperContainer).length !== slides.length;
...
const featuredProducts = (node) => {
    ...
    function handleMobileSwiper() {
        ...
        mobileFeaturedProducts =
            qs(selectors$L.leftSideMobileFeaturedProducts, node) ||
            qs(selectors$L.featuredProducts, node);
...
class Product {
    constructor(node) {
        ...
        this.reviewsHandler = reviewsHandler(
            qs(selectors$G.productReviews, this.container),
            this.container
        );
        ...
        if (variant.featured_media) {
            if (
                ...
                if (this.mobileSwiper) {
                    ...
                    if (targetSlide) {
                        const targetSlideIndex = [...targetSlide.parentElement.children].indexOf(
                            targetSlide
                        );
            ...
            .catch((error) => {
                ...
                if (error && error.message) {
                    if (typeof error.message === 'object') {
                        const sectionID = qs(selectors$G.giftCardRecipientContainer, this.container)
                            .dataset.sectionId;
...
function Navigation(node, headerSection) {
    ...
    const events = [
        ...
        listen(qsa('[data-link]', node), 'focus', (e) => {
            ...
            siblings.forEach((el) => toggleClass(qsa('[data-submenu]', el.parentNode), 'active', el === link));
        }),
...
const filterSidebar = (node) => {
    ...
    function checkForActiveFilters() {
        const activeItems =
            containsCheckedInputs(qsa(sel.filterInputs, container)) || rangeInputsHaveValue();
